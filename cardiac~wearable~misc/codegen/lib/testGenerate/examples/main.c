...
static void main_testGenerate(void);
...
static void main_testGenerate(void)
{
  ...
  /* Initialize function 'testGenerate' input arguments. */
  /* Initialize function input argument 'rrIntervals'. */
  /* Call the entry-point 'testGenerate'. */
  argInit_1x10_real_T(dv);
  testGenerate(dv, &afDetected, &meanRR, &stdRR, &cv);
}
